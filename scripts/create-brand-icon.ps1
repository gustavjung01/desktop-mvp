...
foreach ($size in $sizes) {
    ...
    $drawing.DrawRectangle(
        ...
        $null,
        [Windows.Rect]::new(0, 0, $size, $size))
    $drawing.DrawImage(
        $sourceBitmap,
        [Windows.Rect]::new($left, $top, $width, $height))
    $drawing.Close()

    $rendered = [Windows.Media.Imaging.RenderTargetBitmap]::new(
        $size,
        $size,
        96,
        96,
    ...
    $encoder.Frames.Add([Windows.Media.Imaging.BitmapFrame]::Create($rendered))
